...
export let find_all = async (
  sort: string = "rank",
  order: string = "asc",
  time: string = "last_week"
) => {
  return await Model.App.find({}).sort(create_sort_query(sort, order, time));
};

export let find_approved = async (
...
export let find_approved_lean = async (
  approved: boolean,
  sort: string = "rank",
  order: string = "asc",
  time: string = "last_week"
  ...
  return await Model.App.find(q, { _id: 0 })
    .sort(create_sort_query(sort, order, time)) /*.select('name')*/
    .lean();
};
...
export let create = async (
  ...
  description: string,
  main_account: string,
  accounts: { name: string; account_types: Model.AccountType[] }[],
  logo: string,
  link: string,
  social: {
    discord?: string;
  ...
  tags: string[],
  custom_jsons: string[],
  app_type: string | "app" | "dapp" | "interface"
) => {
  try {
    return await Model.App.create({
      name,
      display_name,
      description,
      main_account,
