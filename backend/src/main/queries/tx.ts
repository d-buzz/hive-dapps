...
import {
  create_arr_of_arrays,
  create_grouped_array,
  create_sum_array_of_group,
} from "../../helpers/convert";

import {
  query,
  TIME_GROUP_ORDER_QUERY,
  TIME_GROUP_QUERY,
  TIMESTAMP,
  convert_grouped,
} from "./query";
...
export let tx_transfers = async (
  ...
  try {
    ...
    let query_incoming = `SELECT COUNT(*), ${TIMESTAMP()} FROM TxTransfers WHERE[to] = '${account}' AND amount_symbol = '${asset}' AND amount >= ${THRESHOLD} ${TIME_GROUP_ORDER_QUERY(
      last_update
    )}`;
    let query_outgoing = `SELECT COUNT(*), ${TIMESTAMP()} FROM TxTransfers WHERE[from] = '${account}' AND amount_symbol = '${asset}' AND amount >= ${THRESHOLD} ${TIME_GROUP_ORDER_QUERY(
      last_update
    )}`;
    ...
    const data = create_sum_array_of_group(
      create_grouped_array(create_arr_of_arrays([data_incoming, data_outgoing]))
    );

    await db_data.create_or_add_both(app, data_type, data, account);
    await db_data.create_or_add_both(
      app,
      `${data_type}_outgoing`,
      data_outgoing,
      account
...
export let tx_custom = async (
  ...
  try {
    ...
    let q = `SELECT COUNT(*), ${TIMESTAMP()} FROM TxCustoms WHERE left(tid, ${
      custom_string.length
    }) = '${custom_string}' ${TIME_GROUP_ORDER_QUERY(last_update)}`;
...
export let tx_meta = async (app: string, meta_name: string, last_update) => {
  try {
    console.log(`TX_META`);
    let q = `SELECT COUNT(*), ${TIMESTAMP(
      "created"
    )} FROM Comments WHERE ISJSON(json_metadata) > 0 AND SUBSTRING(JSON_VALUE(json_metadata, '$.app'), 0, ${
      meta_name.length + 1
    }) = '${meta_name}' ${TIME_GROUP_ORDER_QUERY(last_update, "created")}`;
    const result = await query(q);
