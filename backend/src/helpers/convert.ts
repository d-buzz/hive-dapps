...
export const vests_to_sp = (vests: number | string, global_properties) => {
  if (typeof vests === "string")
    vests = parseFloat(vests.replace(" VESTS", ""));
  return parseFloat(
    (
      global_properties.total_vesting_fund *
      (vests / global_properties.total_vesting_shares)
    ).toFixed(3)
  );
};
...
export let add_values = (values) => {
  ...
  for (let v of values) {
    for (let key of Object.keys(v)) {
      if (!x[key]) x[key] = 0;
      x[key] = Number(x[key]);
      x[key] += v[key] ? Number(v[key]) : 0;
      x[key] = x[key] ? parseFloat(x[key].toFixed(3)) : x[key];
    }
  }
  x = get_change(x);
  return x;
...
export let create_grouped_array = (data, var_name = "timestamp") => {
  return _(data)
    .groupBy((x) => moment.utc(x[var_name]).toISOString())
    .value();
};
